...
export const SearchAndContent:React.FC<ICountriesList> = (props)=>{
    ...
    const selectRegion = (e:React.ChangeEvent<HTMLSelectElement>)=>{
        ...
        pagesNumberEl.current.forEach((item:any)=>{
            ...
            return
            props.themeColor==="light" ?  item.current.style.backgroundColor = "#fff" : item.current.style.backgroundColor = "#1E2C35"
            if(item.current.dataset.number === "1")
                props.themeColor==="light" ? item.current.style.backgroundColor = "lightgrey" : item.current.style.backgroundColor = "#0a3855"
        })
    }
    ...
    if (pagesNumberEl.current.length !== (Math.ceil(countriesList.length)/10)) {
        pagesNumberEl.current = Array(25).fill(0).map((_, i) => pagesNumberEl.current[i] || createRef());
      }
    ...
    return(
        ...
        <CountriesContentWrapper>
            <CountriesContent countriesList={countriesList} pagesNumber={choosePageNumber} handleMoreInfo={handleMoreInfo}/>
        </CountriesContentWrapper>
        <PaginationWrapper themeColor = {props.themeColor}>
            <HiOutlineArrowCircleLeft onClick={()=>decrementPageNumber()} />
                <Pagination pagesNumber = {pagesNumber} handleChoosePageNumber= {handleChoosePageNumber} pagination={props.pagination} choosePageNumber = {choosePageNumber} countriesListLength = {countriesList.length} pagesNumberEl = {pagesNumberEl} themeColor={props.themeColor} />
...
div{
    ...
    width: 30px;
    height: 30px;
    border: 0.1rem solid ${props => props.themeColor === "light" ? "lightgray" : "#fff"};
    color: ${props => props.themeColor === "light" ? "#000" : "#fff"};

}
div:nth-of-type(1){
    background-color: ${props => props.themeColor === "light" ? "#fff" : "#1E2C35"};
    color: ${props => props.themeColor === "light" ? "#000" : "#fff"};  
}
svg{
    display: block;
